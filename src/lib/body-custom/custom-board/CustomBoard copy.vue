 ...
 * @LastEditTime: 2021-09-22 16:34:50
-->
<template>
  <div class="custom-board">
    ...
    <div class="custom-board__medium">
      <button @click="handleOpenImageExtendSelector">打开</button>
      <!-- fabric -->
      <div class="custom-board__canvas-box" ref="canvasBox">
        ...
        <custom-files :list="selectFiles"></custom-files>
      </div>
      <!-- 色彩饱和度调整 -->
...
export default {
  ...
  setup(props, context) {
    const { selectFiles, config, skin } = props;
    ...
    onMounted(() => {
      state.currentAvatar = selectFiles[0].avatar;
    ...
    function renderer() {
      state.canvas.setOption({
        avatar: state.currentAvatar.url,
        chin: state.currentAvatar.chin,
        option: config,
        skin: skin,
        success: () => {
          state.loading = false;
